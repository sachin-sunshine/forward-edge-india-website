export default function Hero() {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-radial-[ellipse_80%_60%_at_50%_60%] from-[#112240] via-[#0A1628] to-transparent" />
      ...
      <div className="absolute top-0 right-0 w-96 h-96 opacity-20 pointer-events-none overflow-hidden">
        <svg viewBox="0 0 384 384" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
          <circle cx="384" cy="0" r="300" stroke="#C9952A" strokeWidth="0.75" fill="none" />
          <circle cx="384" cy="0" r="220" stroke="#C9952A" strokeWidth="0.5" fill="none" />
          <circle cx="384" cy="0" r="140" stroke="#C9952A" strokeWidth="0.5" fill="none" />
        </svg>
      </div>
      ...
      <div className="absolute bottom-0 left-0 w-64 h-64 opacity-10 pointer-events-none overflow-hidden">
        <svg viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
          <circle cx="0" cy="256" r="200" stroke="#C9952A" strokeWidth="0.75" fill="none" />
          <circle cx="0" cy="256" r="130" stroke="#C9952A" strokeWidth="0.5" fill="none" />
        </svg>
      </div>
        ...
        >
          ...
          {[60, 100, 140, 180, 220].map((y) =>
            [80, 120, 160, 200].map((x) => (
              <circle key={`${x}-${y}`} cx={x} cy={y} r="1.5" fill="#C9952A" opacity="0.5" />
            ))
          )}
        </svg>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 md:py-40">
        <div className="max-w-3xl">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.1] tracking-tight mb-6 md:mb-8">
            Empowering{" "}
            <span className="text-[#C9952A]">Governments.</span>
            <br />
            Strengthening{" "}
            <span className="relative inline-block">
              Governance.
              <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-[#C9952A] opacity-60" />
            </span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a
              href="#services"
              className="inline-flex items-center justify-center gap-2 bg-[#C9952A] hover:bg-[#e0aa40] text-[#0A1628] font-bold text-sm px-8 py-4 rounded-sm transition-all duration-200 tracking-wide shadow-lg shadow-[#C9952A]/20 hover:shadow-[#C9952A]/40 hover:-translate-y-0.5"
            >
              Our Services
            ...
            <a
              href="#contact"
              className="inline-flex items-center justify-center gap-2 border border-white/30 hover:border-[#C9952A] text-white hover:text-[#C9952A] font-semibold text-sm px-8 py-4 rounded-sm transition-all duration-200 tracking-wide hover:-translate-y-0.5"
            >
              Get in Touch
            </a>
          </div>
          ...
          <div className="mt-16 md:mt-20 pt-10 border-t border-white/10 grid grid-cols-3 gap-6 max-w-xl">
            {[
              ...
              <div key={stat.label}>
                <div className="text-2xl md:text-3xl font-bold text-[#C9952A]">{stat.value}</div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 opacity-50">
        ...
        <div className="w-0.5 h-8 bg-gradient-to-b from-[#C9952A] to-transparent animate-pulse" />
      </div>
    </section>
  );
}
